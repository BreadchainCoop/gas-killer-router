...
- `AVS_REFERENCE_TARGET`, `AVS_REFERENCE_TARGET_FILE`, `DEMO_TARGET_ADDRESS`, `DEMO_TARGET_FILE`, `DEMO_FACTORY_ADDRESS`, `DEMO_FACTORY_FILE`, `SCHNORR_STAKE_REGISTRY_ADDRESS`: feed the `contracts` block on `GET /avs-metadata` — `chainId`, `avsAddress`, `blsSignatureChecker`, `registryCoordinator`, `schnorrStakeRegistry`, plus the demo contracts the docs point at. Publishing the live pair is what stops a target being wired to a superseded checker, which passes every router-side check and then reverts `InvalidQuorumApkHash` on chain.
  ...
  | `SCHNORR_STAKE_REGISTRY_ADDRESS` | `schnorrStakeRegistry`, overriding the registry `avs_deploy.json` records |
  ...
  `schnorrStakeRegistry` is published under either signature scheme, because the router only serves it and never verifies against it: publishing it while the fleet signs BLS is what lets an integrator deploy a target that accepts both, so a scheme cutover needs no action from them. It comes from `SCHNORR_STAKE_REGISTRY_ADDRESS` or, failing that, from what the operator-set job recorded in `avs_deploy.json`, and publishes only once `nextPossibleMutationBlock()` answers at the address — the call that tells a registry apart from a checker pasted in its place. Provisioning one ahead of the operator set is in the chart's [Schnorr section](helm/gas-killer/README.md#publishing-the-registry-before-a-cutover).
